...
class RequestHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if self.path.startswith('/fetch-sms'):
            ...
            columns = ["id", "content", "subject", "status", "timestamp"]  # Define column names manually
            messages = [dict(zip(columns, row)) for row in cursor.fetchall()]  # Convert each row to a dictionary
            conn.close()
